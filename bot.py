...
async def rates_updater():
    """Обновляет USD/RUB и EUR/RUB каждые 5 минут."""
    while True:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    # EUR/RUB = RUB / EUR (оба относительно USD)
                    if 0 < eur < 2 and rub > 0:
                        config.EUR_RUB_RATE = round(rub / eur, 4)
        except Exception:
            pass
...
def info_keyboard(uid: int):
    ...
    rows = [
        ...
        [
            InlineKeyboardButton(text="⚽ Тотализатор", callback_data="toto_menu"),
        ],
    ]
...
async def main():
    ...
    print(f"   💶 Начальный курс EUR/RUB: {config.EUR_RUB_RATE:.2f} ₽")
